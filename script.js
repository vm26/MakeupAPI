...
async function getBrands(){
    try{
        ...
        array.forEach(ele=>{if(String(ele).indexOf("'")>0)
        {
            ...
            resultarea.innerHTML+=`<h2 class="brands" onclick="getData('${name}')">${ele}</h2><br>`;          
        } else{     
            resultarea.innerHTML+=`<h2 class="brands" onclick="getData('${ele}')">${ele}</h2><br>`;}
        })  ;   
    }
    catch(error){
        resultarea.innerHTML+=`<div class="catch">There is some problem in loading the brands...Please try again later<h2>`;
    }
}
getBrands();
